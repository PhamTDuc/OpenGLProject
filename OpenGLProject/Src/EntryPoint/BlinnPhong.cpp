...
int main()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		modelShading.setBool("blinn", blinn);
		...
		modelShading.setFloat("light.outerCutOff", glm::cos(glm::radians(25.5f)));
...
void processInput(GLFWwindow *window) {
	...
	if (glfwGetKey(window, GLFW_KEY_B) == GLFW_PRESS && !keyPressed) {
		blinn = !blinn;
		keyPressed = true;
	}
